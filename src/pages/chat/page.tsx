...
export default function ChatPage() {
  ...
  /** 모바일에서 "채팅방 정보" 클릭 시 팀 투자 현황 바텀시트 */
  const [mobilePortfolioSheetOpen, setMobilePortfolioSheetOpen] = useState(false);
  ...
  useEffect(() => {
    if (selectedHolding != null)
      setSellQuantityInput(String(selectedHolding.quantity ?? 0));
  ...
  /** 내가 제안한 지정가 대기 주문 (pending + LIMIT) */
  const myPendingLimitOrders = useMemo(
    () =>
      votes.filter(
        (v) =>
          v.status === "pending" &&
  ...
  return (
    <div className="flex-1 min-h-0 overflow-hidden flex flex-col min-w-0 bg-gray-50">
      <main className="flex-1 min-h-0 overflow-hidden flex flex-col w-full px-0 py-0 box-border lg:max-w-7xl lg:mx-auto lg:px-8">
        ...
        {matchingLoading ? (
          ...
          <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
            {/* 왼쪽: 포트폴리오 (데스크톱만) | 오른쪽: 채팅/투표 — 스크롤은 메시지 리스트 1곳만 */}
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] gap-0 lg:gap-6 flex-1 min-h-0 overflow-hidden">
                                ...
                                onClick={async (e) => {
                                  ...
                                  try {
                                    ...
                                    if (res.success) {
                                      ...
                                      setVotes(updated);
                                      setMessages((prev) => {
                                        getChatMessages(groupId).then((list) =>
                                          setMessages(list),
                                        );
                                        return prev;
                                      });
                                        ...
                                        try {
                                          ...
                                          if (res.success) {
                                            await sendTradeMessage(groupId, {
                                              ...
                                              pricePerShare: currentPrice,
                                              totalAmount:
                                                currentPrice * sellQuantity,
                                              reason,
                                              tags: ["매도"],
                                              voteId: res.voteId,
                                            }).catch(() => {});
                                            getChatMessages(groupId).then(
                                              setMessages,
                                            );
                                            const updated =
              ...
              <div className="flex flex-col flex-1 min-h-0 overflow-hidden order-1 lg:order-2 min-w-0">
                ...
                <header className="shrink-0 px-0 lg:px-4 py-2 flex items-center justify-between gap-2 bg-teal-50 border-0 rounded-none shadow-none lg:border lg:border-gray-200 lg:border-b-0 lg:rounded-t-2xl lg:rounded-b-none lg:shadow-sm min-w-0">
                    ...
                    {myRooms.length > 1 && (
                      ...
                      >
                        {myRooms.map((room) => {
                          ...
                          const myTeamNum =
                            user?.teamId != null &&
                            String(user.teamId).startsWith("team-")
                              ? parseInt(
                                  String(user.teamId).replace(/^team-/, ""),
                                  10,
                                )
                              : null;
                          const isMyTeam =
                ...
                <div className="w-full min-w-0 px-0 lg:px-4 py-2 flex justify-center shrink-0 bg-teal-50 border-0 lg:border-x lg:border-b lg:border-gray-200">
                ...
                {/* Row 3+4: 메시지/투표 영역(스크롤) + 입력창(하단 고정) — 모바일 카드 스타일 제거 */}
                <div className="flex flex-col flex-1 min-h-0 w-full min-w-0 overflow-hidden bg-white rounded-none border-0 shadow-none lg:rounded-b-2xl lg:border lg:border-t-0 lg:border-gray-200 lg:shadow-sm">
                  {rightPanelTab === "chat" && (
                    <>
                      {chatError && (
                        <div className="flex-shrink-0 px-0 lg:px-4 py-3 bg-amber-50 border-b border-amber-200">
                          <p className="text-sm text-amber-800">{chatError}</p>
                        </div>
                      )}
                      {groupId != null && !chatError && (
                        <div className="flex-shrink-0 px-0 lg:px-4 py-1.5 border-b border-gray-100 flex items-center gap-2">
                          <span
                      ...
                      <div
                        ref={messageListRef}
                        className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-0 lg:px-4 py-3 pb-4 space-y-4 min-w-0"
                      >
                                  ...
                                  <div className="bg-green-50 rounded-2xl rounded-tl-sm p-4 shadow-sm border border-green-200 max-w-full overflow-hidden break-words">
                                    ...
                                    <p className="text-sm text-gray-800">
                                      <span className="font-semibold">
                                        {msg.executionData.stockName}
                                      </span>{" "}
                                      {msg.executionData.quantity}주 · 체결가{" "}
                                      {formatCurrency(
                                        msg.executionData.executionPrice,
                                      )}
                                    </p>
                                    {msg.userNickname &&
                                      msg.userNickname !== "시스템" && (
                                        <p className="text-xs text-gray-500 mt-1">
                                          제안: {msg.userNickname}
                                        </p>
                                      )}
                        ...
                        {showScrollToBottom && (
                          <div className="hidden lg:flex sticky bottom-4 justify-center">
                            <button
                              type="button"
                              onClick={() => {
                      ...
                      <div className="shrink-0 sticky bottom-0 z-20 bg-white/95 backdrop-blur border-t border-gray-200 px-0 lg:px-4 py-3 pb-14 md:pb-3">
                        {isTournamentEnded ? (
                          <button
                            type="button"
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setMobilePortfolioSheetOpen(false);
                }}
                className="w-10 h-10 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg hover:bg-gray-100 cursor-pointer"
                aria-label="닫기"
              >
      ...
      {/* 모바일(<lg): 팀 투자 현황 바텀시트 — 데스크톱과 동일한 팀 투자 현황 콘텐츠 */}
      <BottomSheet
        open={mobilePortfolioSheetOpen}
        onClose={() => {
          setMobilePortfolioSheetOpen(false);
          setShowGroupInfoModal(false);
        }}
      ...
      >
        <div className="p-4 space-y-4">
          ...
          {/* 보유 종목 리스트 (모바일 시트에서는 요약만: 종목명·수량·평가·수익률) */}
          <div className="space-y-2">
            <h3 className="text-sm font-bold text-gray-900">
              보유 종목
            </h3>
            {(groupPortfolioData?.holdings ?? []).map((holding) => {
              ...
              const profitLossPercentage =
                cost > 0 ? (profitLoss / cost) * 100 : 0;
              const isHoldingProfit = profitLoss >= 0;
              return (
                <div
                  key={holding.id}
                  className="rounded-xl p-3 border border-gray-200 bg-white"
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <p
                      className={`text-xs font-semibold flex-shrink-0 ml-2 ${
                        isHoldingProfit ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      <span className="text-gray-500 font-normal">
                        매수대비{" "}
